...
void AFRL_Driver::commsCallBack(std_msgs::UInt8 comm) {
    ...
    switch(s) {
        case TAG_NULL:
            ROS_INFO_STREAM("TAG_NULL: " << comm);
            break;
        case TAG_FOUND:
            ROS_INFO_STREAM("TAG_FOUND: " << comm);
            break;
        case TAG_LOST:
            ROS_INFO_STREAM("TAG_LOST: " << comm);
            break;
    }
}
...
bool AFRL_Driver::turnOdom(double angle, bool clockwise, bool backtrack) {
    move(0, 0); // Stop for a moment and sleep for one second.
    sleep(1);
    ...
    clockwise ? base_cmd.angular.z = -0.5 : base_cmd.angular.z = 0.5;
...
void  AFRL_Driver::spin() {
    ros::Rate rate(11); // Specify the looprate in Hz
